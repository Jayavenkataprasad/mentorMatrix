      ...
      const fetchQuizData = async () => {
        ...
        try {
          ...
          if (questionsData.length === 0) {
            ...
            setError(`No questions available for entry ${entryId}. Your mentor may not have added any questions yet.`);
            setLoading(false);
            return;
          }
          ...
          if (error.response?.status === 403) {
            if (error.response.data.error && error.response.data.error.includes('deadline')) {
              setError(error.response.data.error);
            } else if (error.response.data.error && error.response.data.error.includes('already attempted')) {
              setError('You have already attempted this quiz. You can only take it once.');
              ...
              setError('Access denied. Please make sure you are logged in as the correct student.');
            }
          } else if (error.response?.status === 404) {
            setError(`Entry ${entryId} not found. Please check if this entry belongs to you and try again.`);
          } else if (error.response?.status === 500) {
            setError('Server error occurred while loading questions. Please try again later.');
          } else {
            setError(`Failed to load questions for entry ${entryId}. Please check your internet connection and try again.`);
          }
        } finally {
          setLoading(false);
        }
